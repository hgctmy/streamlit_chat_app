...
def initfn():
    with kijisentaku.container():
        ...
        if st.button("記事を選択し，ワーカ名を入力してエンターキーを押しました．", key='first'):
            # st.session_state.workbook = gc.create("base_" + st.session_state.worker)
            # ws = st.session_state.workbook.get_worksheet(0)
            # ws.append_row(["選ばれた記事", st.session_state.kiji])
            # ws.append_row(["質問候補1", "質問候補2", "質問候補3", "選ばれた質問番号", "選ばれた質問(手入力の質問)"])
            # 説明してもらう文章
...
def click(i):
    with chat_placeholder.container():
        ...
        st.session_state.assistant1.append({"role": "user", "content": choices[i]})
        st.session_state.dialog.append("質問者：" + choices[i])
        ...
        st.session_state.assistant1.append({"role": "assistant", "content": answer})
