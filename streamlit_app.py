...
with kijisentaku.container():
    kijitext = st.radio("選んでください", (kijilist))
    kiji = kijilist.index(kijitext)
    if not st.checkbox("記事を選択しました．"):
        st.stop()
kijisentaku.markdown(kijitext)
...
while True:
    ...
    with st.container():
        ...
        with placeholder.container():
            q = st.radio("選んでください", (choices), key=i)
            i += 1
            check = st.checkbox('次へ', key=i)
            i += 1
            end = st.checkbox("終了する", key=i)
            i += 1
            if end:
                break
            if not check:
    ...
    if "ません" in completion.choices[0].message.content:
        st.text("検索しています")
        result = kensaku.searchfromquestion(q)
