...
articles = ["①Brightest-ever cosmic explosion solved but new mysteries sparked", "②Can AI help solve Japan’s labour shortages?", "③Baltimore sues owner and manager of 'unseaworthy' Dali over bridge collapse", "④Spain to axe 'golden visas' scheme"]
articlelist = st.empty()
...
def initfn():
    ...
    with articlelist.container():
        st.session_state.kijitext = st.radio("Please select the specified article", (articles))
        st.session_state.kiji = articles.index(st.session_state.kijitext)
        ...
        if len(st.session_state.worker) > 0:
            if st.button("I selected an article, typed in the worker id.", key='first'):
                ...
                articlelist.markdown(st.session_state.kijitext)
                with chat_placeholder.container():
                    ...
                    button_placeholder.write("generating questions ...")
                    # 初めの質問候補を生成
